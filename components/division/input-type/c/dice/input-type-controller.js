export default function inputTypeController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  ...
  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.division.types.dice');
  ...
  $scope.diceContainer = RequireImages.get($scope.getImageContext(), questionData.diceContainer);

  console.log('diceImage', $scope.diceImage);
  console.log('diceImage', $scope.diceContainer);
  var value = '';
  ...
  function init() {
    ...
    $scope.diceImage = $scope.diceImages[$scope.answer];
