export default function inputTypeController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  ...
  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.division.types.reverse-inverse');

  var questionData = gameData.home.question;
  $scope.questionText = questionData.hint.text;
  $scope.header = gameData.home.header;
  $scope.footerText = questionData.footerText;
  $scope.headerText = questionData.headerText;
  $scope.flag = true;
  ...
  $scope.multiplication = RequireImages.get($scope.getImageContext(), questionData.multiplication);
  $scope.division = RequireImages.get($scope.getImageContext(), questionData.division);
  ...
  function init() {
    ...
    $scope.firstNumber = $scope.generateRandomNumber(maxLimit, minLimit);
    $scope.secondNumber = $scope.generateRandomNumber(maxLimit1, minLimit1);
    $scope.answer = $scope.firstNumber * $scope.secondNumber;
    $scope.result = $scope.answer / $scope.firstNumber;
