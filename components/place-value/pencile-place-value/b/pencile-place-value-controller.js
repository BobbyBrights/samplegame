export default function pencilePlaceValueController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  'ngInject';

  $scope.gamePageView = 'pencile-place-value.b';

  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.place-value.types.pencile-place-value');
  ...
  function init() {
    ...
    $scope.result = [];
    $scope.groupOfPencile = _.range(generateRandomNumber(1, 9));
    $scope.result.push($scope.groupOfPencile.length);
    $scope.penciles = _.range(generateRandomNumber(1, 9));
    $scope.result.push($scope.penciles.length);
  ...
  function check($event) {
    $event.preventDefault();
    if (isCorrect()) {
      angular.element('#input-2').css({'visibility': 'visible'});
      var answer = (10 * answers[0]) + answers[1];
      angular.element('#input-2').val(answer);
      $scope.flag = 1;
