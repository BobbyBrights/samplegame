export default function circlePlaceValueController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  'ngInject';

  $scope.gamePageView = 'circle-place-value.b';

  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.place-value.types.circle-place-value');
  ...
  $scope.pencileBunch = RequireImages.get($scope.getImageContext(), questionData.pencileBunch);
  $scope.pencile = RequireImages.get($scope.getImageContext(), questionData.pencile);
  $scope.images = RequireImages.get($scope.getImageContext(), gameData.home.images);
  $scope.blueCircle = RequireImages.get($scope.getImageContext(), questionData.abacus.blueAbacus);
  $scope.darkmagentaCircle = RequireImages.get($scope.getImageContext(), questionData.abacus.darkmagentaAbacus);
  ...
  function init() {
    ...
    $scope.blueAbacus = _.range(generateRandomNumber(1, 9));
    $scope.result.push($scope.blueAbacus.length);
    $scope.darkmagentaAbacus = _.range(generateRandomNumber(1, 9));
    $scope.result.push($scope.darkmagentaAbacus.length);
