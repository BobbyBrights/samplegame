export default function circleboxPlaceValueController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  'ngInject';

  $scope.gamePageView = 'circlebox-place-value.b';

  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.place-value.types.circlebox-place-value');
  ...
  $scope.groupOfCircle = RequireImages.get($scope.getImageContext(), questionData.groupOfCircle);
  $scope.circles = RequireImages.get($scope.getImageContext(), questionData.circles);
  ...
  function init() {
    ...
    $scope.result = [];
    $scope.groupOfCircles = _.range(generateRandomNumber(1, 9));
    $scope.result.push($scope.groupOfCircles.length);
    $scope.circle = generateRandomNumber(1, 9);

    var noOfCircle = $scope.circle;
    $scope.result.push(noOfCircle);
