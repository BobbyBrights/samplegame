export default function toTwentyController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  'ngInject';

  $scope.gamePageView = 'to-twenty.b';

  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.addition.types.to-twenty');
...
//Disable special characters and alphabets
  ...
  function init() {
    ...
    $scope.firstNumber = $scope.generateRandomNumber(10, 0);
    $scope.secondNumber = $scope.generateRandomNumber(10, 0);
    ...
    $scope.secondCount = _.range(0, $scope.secondNumber);
    $scope.flag = -1;
