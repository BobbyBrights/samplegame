export default function blockController($scope, $state, $stateParams, GameData, RequireImages, $timeout) {
  'ngInject';

  $scope.gamePageView = 'block.b';

  var gameData = GameData.getCamelCase('levels.' + $scope.level + '.categories.' +
    $stateParams.category + '.games.data-graphs.types.block');
  ...
  $scope.iconImages = RequireImages.get($scope.getImageContext(), questionData.boxImages);
  ...
  $scope.numberOfInputs = _.range(0, 6);
  $scope.inputs = [];
  $scope.count=0;
  ...
  function displayIcons() {
    numbersOfImage = _.take(_.shuffle(_.range(1, 11)), 6);
